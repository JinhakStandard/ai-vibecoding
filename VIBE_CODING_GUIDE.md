...
### 6.8 계획 수립 프레임워크 (Plan Granularity) (v2.2)

바이브 코딩에서 **구현보다 촘촘한 계획 수립이 중요합니다**. 계획 없이 바로 코딩하면 방향이 흐려지고, 과한 계획은 시간 낭비입니다. 작업 복잡도에 맞는 적절한 계획 입도(granularity)를 선택하세요.

#### 계획 입도 4단계

| 레벨 | 이름 | 기준 | 상세도 | 도구 |
|------|------|------|--------|------|
| L1 | 스케치 | 단일 파일, 명확한 지시 | 변경 파일 + 핵심 변경 1줄 | (없음, 바로 구현) |
| L2 | 아웃라인 | 2~4파일, 중간 복잡도 | 파일별 변경 + 순서 + 완료 조건 | `EnterPlanMode` |
| L3 | 상세 계획 | 5파일+, 아키텍처 결정 | 6개 필수 항목 전부 포함 | `/deep-plan` |
| L4 | 설계 문서 | 신규 모듈/서비스 | L3 + 데이터 모델 + API 스펙 + ADR | `/deep-plan` + `.ai/DECISIONS.md` |

#### L3/L4 계획서 필수 6개 항목

1. **요구사항 분석** - 명시적/암묵적 요구사항, 범위 밖 항목
2. **영향도 분석** - 수정 파일 목록, 기존 의존성, 사이드이펙트
3. **단계별 구현 계획** - 독립 테스트 가능 단위, 단계당 1~3파일, 완료 조건
4. **기술적 결정** - 선택한 방식 + 고려한 대안 + 트레이드오프
5. **위험 요소** - 실패 가능성, 성능, 보안
6. **테스트 전략** - 단계별 검증 방법, 엣지 케이스

#### 비평 체크리스트 (C1~C7)

`/deep-plan` 사용 시 Critic 에이전트가 자동 평가하지만, 수동 점검용으로도 활용 가능합니다:

| 코드 | 관점 | 확인 포인트 |
|------|------|-----------|
| C1 | 실현 가능성 | 현재 코드베이스에서 구현 가능한가? |
| C2 | 완전성 | 누락된 파일/설정/에러 처리가 없는가? |
| C3 | 과잉 설계 방지 | YAGNI 위반, 불필요한 추상화가 없는가? |
| C4 | 사용자 의도 부합 | 원래 요청과 어긋나지 않는가? |
| C5 | 기존 패턴 준수 | 프로젝트 컨벤션/JINHAK 표준을 따르는가? |
| C6 | 보안 | 금지 패턴 12개에 해당하지 않는가? |
| C7 | 단계 분해 품질 | 각 단계가 독립 테스트 가능하며 크기가 적절한가? |

#### 수렴 기준 (v2.3 가중치 + Hard Gate)

- **통과**: 가중 비율 `가중총점 / 가중만점 ≥ 80%` AND 치명적 문제 0건
- **C6 Hard Gate**: 보안/비즈니스 유형 작업은 C6 보안 점수 **8/10 이상** 필수 (가중 비율 충족과 별개)
- **최대 반복**: 3라운드 (초기 + 수정 2회)
- **정체 감지**: 라운드 간 가중 비율 개선 +5%p 미만 시 사용자 위임
- **NightBuilder 분기**: 무인 환경에서 미수렴 시 `.ai/PENDING_PLANS.md`에 보류 저장 후 다음 태스크 진행
...
> 유형별 가중치로 만점이 달라지므로 절대 점수가 아닌 **비율(%)**로 수렴 판정합니다.
...
요구사항 → [복잡도 자동 판정]
              │
              ├─ L1~L2: 바로 구현 또는 EnterPlanMode
              │
              └─ L3+ 해당 → "/deep-plan 추천" 안내 (강제 아님)
                    │
                    ├─ [사용자 승인] → /deep-plan → 가중 비평 + Hard Gate
                    │                                    │
                    │                                    ├─ 수렴 → 바로 구현 or /orchestrate
                    │                                    └─ 미수렴 → 사용자 위임 (NightBuilder: 보류 저장)
                    │
                    └─ [사용자 거절] → 일반 Plan 모드로 계속
```
