...
**2번 `/orchestrate`로 병렬 구현 선택 시 상태 전달 (State Contract):**

`/orchestrate` 호출 시 다음 정보를 자동 전달합니다:

| 항목 | 출처 | 전달 방식 |
|------|------|----------|
| 계획서 경로 | `.ai/plans/YYYY-MM-DD_HHmm_[작업요약].md` | 인자로 전달 |
| 작업 유형 태그 | 0단계 분류 결과 | 인자로 전달 |
| 단계별 구현 계획 | 계획서의 "단계별 구현 계획" 섹션 | 계획서 파일 참조 |
| C6 Hard Gate 결과 | 4단계 수렴 판정 결과 | 인자로 전달 |

전달 형식:
```
/orchestrate 계획서=.ai/plans/[파일명] 유형=[태그] C6=[통과/해당없음]
```
...
- "이 작업은 단순하므로 Critic 비평이 불필요합니다" → /deep-plan이 호출된 이상 Planner-Critic 전체 루프를 실행합니다. 단순한 작업이면 애초에 /deep-plan을 사용하지 않아야 합니다.
- "이전 라운드에서 거의 수렴했으므로 재비평 없이 통과 처리합니다" → 가중 비율 80% 미만이면 수렴이 아닙니다. 기준은 예외 없이 적용합니다.
- "C6 Hard Gate는 이 작업에 과도합니다" → 보안/비즈니스 유형 판정은 0단계에서 자동 분류됩니다. 유형 판정이 잘못되었다면 사용자에게 확인을 구합니다.
