...
브랜치와 무관하게 표준을 배포할 수 있는 다중 배포 채널을 구축합니다.

### 추가
- `package.json` — npm 패키지 설정 (`npx jinhak-ai-standard` CLI)
- `bin/cli.cjs` — CLI 진입점 (apply, info, link, help 4개 명령)
...
기존 v2.5 프로젝트에서 업데이트 시:
1. `/apply-standard` 실행 — 스킬이 자동으로 npm/Release 폴백 로직을 포함
2. 또는 `npx jinhak-ai-standard` 으로 직접 적용

### npm 퍼블리시 설정 (관리자용)
1. [npmjs.com](https://www.npmjs.com) 계정 생성 및 로그인
2. Access Token 생성 (Automation 타입)
3. GitHub repo Settings → Secrets → `NPM_TOKEN` 으로 등록
4. 이후 `git tag v2.6 && git push origin v2.6` 하면 자동 퍼블리시
