...
**신규 (개선 12~13: 구글 멀티 에이전트 패턴 반영)**
- State Contract: `/deep-plan` → `/orchestrate` 파이프라인에서 계획서 경로, 작업 유형, C6 결과를 명시적으로 전달하는 상태 계약 도입
- 스킬 조합 가이드 (Composite Pattern): 작업 유형별 최적 스킬 조합 의사결정 매트릭스 + 파이프라인 흐름도
