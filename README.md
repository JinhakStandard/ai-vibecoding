...
[QUICK_START_PROMPT.md](./QUICK_START_PROMPT.md)의 프롬프트를 Claude Code에 붙여넣으면 됩니다.

> 신규/기존 프로젝트 모두 동일한 프롬프트를 사용합니다.

### 방법 3: 글로벌 Hook 설치 (최초 1회)

글로벌 Hook을 설치하면 **모든 프로젝트**에서 Claude Code 세션 시작 시 표준 적용 여부가 자동 감지됩니다.

```bash
npx jinhak-ai-standard link
...
├── package.json                  # npm 패키지 설정 (npx jinhak-ai-standard)
├── bin/cli.cjs                   # CLI 진입점 (apply, info, link 명령)
├── .github/workflows/release.yml # GitHub Releases 자동 생성 (태그 push 시)
