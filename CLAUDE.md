...
Claude Code는 프로젝트별로 **대화 간 지속되는 자동 메모리**를 지원합니다. 이를 활용하면 매 세션마다 컨텍스트를 다시 설명하지 않아도 됩니다.

**메모리 저장 경로:**
```
~/.claude/projects/<프로젝트-경로-해시>/memory/
├── MEMORY.md          # 메인 메모리 파일 (매 대화 자동 로드)
├── patterns.md        # 코드 패턴 및 컨벤션
├── debugging.md       # 반복 디버깅 인사이트
└── architecture.md    # 아키텍처 결정 사항
```

**MEMORY.md 핵심 규칙:**
- 매 대화 시작 시 자동으로 컨텍스트에 로드됨
- **200줄 초과 시 뒷부분이 잘림** → 200줄 이내로 유지, 상세 내용은 서브파일로 분리
- 주제별로 정리 (시간순 X, 토픽별 O)

**저장해야 할 것:**
- 여러 세션에 걸쳐 확인된 안정적인 패턴과 컨벤션
- 핵심 아키텍처 결정, 중요 파일 경로, 프로젝트 구조
- 사용자의 워크플로우/도구/소통 스타일 선호
- 반복되는 문제의 해결책 및 디버깅 인사이트

**저장하지 말아야 할 것:**
- 세션별 맥락 (현재 작업 상세, 진행 중인 작업, 임시 상태)
- 불완전한 정보 (프로젝트 문서와 대조 확인 전)
- CLAUDE.md 지침과 중복되거나 충돌하는 내용
- 추측성 또는 단일 파일 읽기에서 나온 미검증 결론

**Auto Memory vs `.ai/` 폴더 역할 구분:**

| 구분 | Auto Memory (`memory/`) | `.ai/` 폴더 |
|------|------------------------|------------|
| 대상 | AI 개인 기록 | 팀 공유 + Git 추적 |
| 형식 | 자유 형식 | 정해진 템플릿 |
| 갱신 | AI가 자동 기록 | 세션 종료 시 수동 기록 |
| 보존 | 로컬만 (git 제외) | Git에 커밋됨 |
| 용도 | 패턴·인사이트 축적 | 스프린트·의사결정 공유 |

> **사용자 요청 시 즉시 저장**: "이거 기억해줘", "항상 bun 써줘" 등의 요청은 여러 세션 확인 없이 바로 `MEMORY.md`에 저장합니다.
...
| `WorktreeCreate` | Git Worktree 생성 시 | - | 격리 환경 초기화에 활용 (v2.1) |
| `WorktreeRemove` | Git Worktree 삭제 시 | - | 격리 환경 정리에 활용 (v2.1) |
...
여러 에이전트를 팀으로 구성하여 병렬/분산 작업을 수행할 수 있습니다. `/orchestrate` 스킬로 간편하게 시작할 수 있습니다.
...
```
TeamCreate → TaskCreate(들) → Task 할당(TaskUpdate) → 팀원 작업
     ↓                                                      ↓
 팀 설정 완료                                        TaskUpdate(완료)
                                                           ↓
...
```
→ `~/.claude/teams/{team-name}/config.json`과 `~/.claude/tasks/{team-name}/` 자동 생성

**2. 태스크 생성 및 할당:**
```
TaskCreate({ subject: "API 구현", description: "...", activeForm: "API 구현 중" })
TaskUpdate({ taskId: "1", owner: "backend-agent", status: "in_progress" })
```

**3. 팀원 메시지 수신:**
- 팀원의 메시지는 **자동으로 배달됨** → 폴링/체크 불필요
- 팀원이 보내는 JSON 상태 메시지(`{"type":"idle",...}`) 사용 금지 → 일반 텍스트로 소통

**4. 팀원 유휴(Idle) 상태:**
- 팀원은 매 턴 종료 후 자동으로 idle 상태가 됨 → **정상 동작**
- Idle = 대기 중, 메시지 전송 시 즉시 깨어남
- Idle 알림이 오면 반응하지 않아도 됨 (단, 새 작업 할당 시 메시지 전송)

**5. 팀 종료:**
```
SendMessage({ type: "shutdown_request", recipient: "팀원이름", content: "작업 완료" })
// 팀원이 approve 후 → TeamDelete()
```

#### 6.4.2 SendMessage 타입

| 타입 | 용도 | 필수 파라미터 |
|------|------|-------------|
| `message` | 특정 팀원에게 1:1 메시지 | `recipient`, `content`, `summary` |
| `broadcast` | 전체 팀원에게 동시 발송 (비용↑, 꼭 필요할 때만) | `content`, `summary` |
| `shutdown_request` | 팀원에게 종료 요청 | `recipient`, `content` |
| `shutdown_response` | 종료 요청에 응답 (approve/reject) | `request_id`, `approve` |
| `plan_approval_response` | 팀원의 Plan 승인/반려 | `request_id`, `recipient`, `approve` |

> **중요**: 팀원은 `target_agent_id` 아닌 **name**으로 참조합니다. 팀 config 파일에서 확인: `~/.claude/teams/{team-name}/config.json`

#### 6.4.3 내장 Task 시스템

Agent Teams는 공유 Task List로 작업을 조율합니다:

| 도구 | 역할 |
|------|------|
| `TaskCreate` | 새 태스크 생성 (status: pending으로 시작) |
| `TaskUpdate` | 상태 변경, 소유자 지정, 의존성 설정 |
| `TaskList` | 전체 태스크 목록 조회 (ID 오름차순 우선 처리) |
| `TaskGet` | 특정 태스크 상세 정보 조회 |

**태스크 의존성 예시:**
```
TaskUpdate({ taskId: "3", addBlockedBy: ["1", "2"] })
// → 태스크 1, 2 완료 전까지 태스크 3 시작 불가
```

#### 6.4.4 Background 에이전트

오래 걸리는 작업은 백그라운드로 실행하고 완료 시 알림을 받습니다:

```
Task({ ..., run_in_background: true })
// → 완료 시 자동 알림 수신, 폴링 불필요
```

**에이전트 Resume (이전 컨텍스트 유지):**
```
Task({ ..., resume: "이전-agent-id" })
// → 이전 실행의 전체 대화 컨텍스트가 보존된 채로 재개
```

#### 6.4.5 적합한 작업 vs 부적합한 작업

**적합한 작업 (병렬화 효과 큼):**
- 코드베이스 전체 리뷰 (파일/모듈별 독립 분석)
- 다중 파일 동시 분석/검색
- 독립적인 기능의 병렬 구현 (프론트엔드 + 백엔드)
- 대규모 리팩토링 사전 조사
...
복잡한 구현 전에 Plan 모드를 활용하여 설계를 먼저 검토하고 사용자 승인을 받습니다.

**Plan 모드 사용 기준 (언제 써야 하는가):**
- 3개 이상 파일을 수정하는 작업
- 여러 가지 구현 방법이 존재하는 경우
- 아키텍처 결정이 필요한 신규 기능
- 요구사항이 불명확하여 탐색이 필요한 경우
- `.ai/DECISIONS.md`에 기록할 수준의 기술 결정

**Plan 모드 사용 안 해도 되는 경우:**
- 단순 오타/버그 수정 (1~2줄)
- 명확한 지시가 있는 단순 기능 추가
...
팀원이 `plan_mode_required` 설정으로 실행된 경우, 팀원이 `ExitPlanMode`를 호출하면 팀 리더에게 승인 요청이 자동 전달됩니다:

```
// 팀 리더가 받는 메시지 (자동 JSON)
{ "type": "plan_approval_request", "requestId": "abc-123", ... }

// 승인:
SendMessage({ type: "plan_approval_response", request_id: "abc-123",
              recipient: "팀원이름", approve: true })

// 반려 (수정 지시):
SendMessage({ type: "plan_approval_response", request_id: "abc-123",
              recipient: "팀원이름", approve: false,
              content: "API 호출 에러 처리 추가해주세요" })
```

#### Task 관리 시스템

Claude Code 내장 Task 도구로 복잡한 작업을 추적합니다:

| 도구 | 용도 |
|------|------|
| `TaskCreate` | 새 태스크 생성 (`subject`, `description`, `activeForm` 필수) |
| `TaskUpdate` | 상태/소유자/의존성 변경 (`in_progress` → `completed`) |
| `TaskList` | 전체 태스크 목록 조회 |
| `TaskGet` | 태스크 상세 정보 조회 |

**태스크 상태 흐름:** `pending` → `in_progress` → `completed` (또는 `deleted`)

**`.ai/CURRENT_SPRINT.md`와 연계:** 내장 Task 시스템은 세션 내 추적용, `.ai/CURRENT_SPRINT.md`는 팀 공유/Git 추적용으로 병행 사용
...
### 6.8 Git Worktree 격리 개발

Git Worktree를 사용하면 **메인 코드베이스에 영향 없이** 독립된 환경에서 작업할 수 있습니다.

#### EnterWorktree 사용법

사용자가 "worktree에서 작업해줘", "worktree 만들어줘" 등을 요청할 때 활성화합니다:

```
EnterWorktree({ name: "feature-auth" })
```
→ `.claude/worktrees/feature-auth/` 하위에 새 브랜치 기반의 격리 저장소 생성
→ 세션이 격리된 worktree로 전환됨
→ 세션 종료 시 변경 없으면 **자동 삭제**, 변경 있으면 브랜치/경로 반환

> **주의**: 사용자가 명시적으로 "worktree"를 언급할 때만 사용. 일반 브랜치 작업이나 기능 개발 요청에는 사용하지 않음.

#### 에이전트 Worktree 격리

서브에이전트를 격리된 worktree에서 실행하면 안전하게 독립 작업 가능:

```
Task({
  subagent_type: "general-purpose",
  prompt: "대규모 리팩토링 수행",
  isolation: "worktree"   // 격리된 저장소 사본에서 실행
})
```
- 에이전트가 변경을 만들면 worktree 경로와 브랜치가 반환됨
- 변경이 없으면 worktree 자동 정리
    ...
    "WorktreeCreate": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "node -e \"console.log('[WORKTREE] 격리 환경 생성됨')\""
        }]
      }
    ],
    "WorktreeRemove": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "node -e \"console.log('[WORKTREE] 격리 환경 정리됨')\""
        }]
      }
    ]
  }
...
| 상황 | Worktree 활용 |
|------|-------------|
| 대형 리팩토링 (되돌리기 가능하게) | 권장 |
| 위험한 마이그레이션 스크립트 테스트 | 권장 |
| Agent Teams 독립 기능 병렬 구현 | `isolation: "worktree"` 활용 |
| 일반 기능 개발 | 불필요 |
| 단순 버그 수정 | 불필요 |
