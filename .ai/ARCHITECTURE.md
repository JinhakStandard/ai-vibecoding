...
│   ├── scripts/
│   │   └── session-briefing.cjs  # 세션 브리핑 자동화
│   └── skills/                  # 슬래시 명령어
│       ├── apply-standard/
│       ├── commit/
│       ├── review-pr/
...
│       └── test/
├── scripts/                     # 유틸리티 스크립트
│   ├── install-global-hook.cjs   # 글로벌 Hook 설치
│   └── batch-apply.cjs           # 일괄 표준 적용
└── templates/                   # 프로젝트 템플릿
    ├── project-claude.md
...
JabisStandard (표준 저장소)
    ↓ /apply-standard 또는 batch-apply.cjs
대상 프로젝트
    ├── CLAUDE.md (메타 태그 + 프로젝트 규칙)
    ├── .claude/settings.json (권한 + Hook)
    ├── .claude/scripts/session-briefing.cjs
    └── .ai/ (세션 기록, 스프린트, 의사결정)
```
...
| session-briefing.cjs | 세션 시작 시 컨텍스트 자동 로드 |
